...
impl MessageHandlingView for View {
    ...
    fn handle_message(&mut self, message: &Message) -> crate::Result<iced::Command<Message>> {
        if let Message::Interaction(Interaction::ViewInteraction(stringified)) = message {
            let local_interaction: LocalViewInteraction =
    ...
    fn data_container<'a>(&'a mut self, color_palette: ColorPalette) -> Container<'a, Message> {
        ...
        let mut my_wallet_table_row = {
            ...
            TableRow::new(row).inner_row_height(24).on_press(move |_| {
                Message::Interaction(Interaction::ViewInteraction(
                    serde_json::to_string(&LocalViewInteraction::SelectMode(Mode::Catalog))
                        .unwrap(),
                ))
            })
        };
        ...
        let mut catalog_mode_button: Button<Interaction> = Button::new(
            &mut self.state.catalog_mode_btn,
        ...
        let mut install_mode_button: Button<Interaction> = Button::new(
            &mut self.state.install_mode_btn,
        ...
        let mut settings_mode_button: Button<Interaction> = Button::new(
            &mut self.state.settings_mode_btn,
            Text::new(localized_string("settings"))
                .horizontal_alignment(alignment::Horizontal::Center)
                .size(DEFAULT_FONT_SIZE),
        )
        .on_press(Interaction::ViewInteraction(
            serde_json::to_string(&LocalViewInteraction::SelectMode(Mode::Settings)).unwrap(),
        ));

        let mut about_mode_button: Button<Interaction> = Button::new(
            &mut self.state.about_mode_btn,
        ...
        match self.state.mode {
            Mode::Install => {
                ...
                settings_mode_button =
                    settings_mode_button.style(style::DefaultButton(color_palette));
            }
            Mode::Catalog => {
                catalog_mode_button =
                    catalog_mode_button.style(style::SelectedDefaultButton(color_palette));
                install_mode_button =
                    install_mode_button.style(style::DefaultButton(color_palette));
                about_mode_button = about_mode_button.style(style::DefaultButton(color_palette));
                settings_mode_button =
                    settings_mode_button.style(style::DefaultButton(color_palette));
            }
