...
pub fn data_container<'a>(
    ...
    let name_header_container =
        Container::new(name_header).style(grin_gui_core::theme::container::Container::BrightForeground(color_palette));

    let chain_header = Text::new(localized_string("type")).size(DEFAULT_SUB_HEADER_FONT_SIZE);
    let chain_header_container =
        Container::new(chain_header).style(grin_gui_core::theme::container::Container::BrightForeground(color_palette));
