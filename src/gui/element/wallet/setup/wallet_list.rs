...
pub fn data_container<'a>(
    ...
    let title_container =
        Container::new(title).style(grin_gui_core::theme::container::Container::BrightBackground).padding(iced::Padding::from([
            0,               // top
            0,               // right
            0,               // bottom
            5,               // left
    ...
    let name_header_container =
        Container::new(name_header).style(grin_gui_core::theme::container::Container::BrightForeground);

    let chain_header = Text::new(localized_string("type")).size(DEFAULT_SUB_HEADER_FONT_SIZE);
    let chain_header_container =
        Container::new(chain_header).style(grin_gui_core::theme::container::Container::BrightForeground);
    ...
    let table_header_container = Container::new(table_header_row)
        ...
        .style(grin_gui_core::theme::container::Container::PanelForeground);
    ...
    for (pos, w) in config.wallets.iter().enumerate() {
        ...
        let mut wallet_name_container =
            Container::new(wallet_name).style(grin_gui_core::theme::container::Container::HoverableForeground);

        let mut wallet_chain_container =
            Container::new(chain_name).style(grin_gui_core::theme::container::Container::HoverableForeground);
    ...
    let table_container = Container::new(table_colummn)
        .style(grin_gui_core::theme::container::Container::PanelBordered)
        .height(Length::Fill)
        .padding(1);
