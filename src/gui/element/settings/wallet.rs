use {
    ...
    grin_gui_core::theme::{Button, Column, Container, Element, PickList, Row, Scrollable, Text, TextInput},
...
pub fn handle_message(
    ...
    let mut check_mixnet_config = || {
        if grin_gui.config.mixnet_keys.is_none() {
            ...
            grin_gui.config.mixnet_keys.as_mut().unwrap().push(String::new());
            grin_gui.config.mixnet_keys.as_mut().unwrap().push(String::new());
            grin_gui.config.mixnet_keys.as_mut().unwrap().push(String::new());
        }
    };
...
pub fn data_container<'a>(
    ...
    let mw_mixnet_address_column = {
        let mw_mixnet_address_container =
            Container::new(Text::new(localized_string("mw-mixnet-addresses")).size(DEFAULT_FONT_SIZE))
                .style(grin_gui_core::theme::ContainerStyle::NormalBackground);
