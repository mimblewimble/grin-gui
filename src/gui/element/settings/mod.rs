...
pub fn data_container<'a>(
	...
	match state.mode {
		Mode::Wallet => {
			...
			node_button = node_button.style(grin_gui_core::theme::ButtonStyle::Primary);
			general_button = general_button.style(grin_gui_core::theme::ButtonStyle::Primary);
		}
		Mode::Node => {
			wallet_button = wallet_button.style(grin_gui_core::theme::ButtonStyle::Primary);
	...
	match state.mode {
		...
		Mode::Node => {
			wrapper_column = wrapper_column.push(node::data_container(node_settings_state))
		}
		Mode::General => {
			wrapper_column =
				wrapper_column.push(general::data_container(general_settings_state, config))
		}
	}
